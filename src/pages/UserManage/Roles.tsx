...
const Roles: React.FC = () => {
  ...
  const addOK = async () => {
    ...
    request.post('/api/user/add_role', {
      header: {headers},
  ...
  const deleteRow = (id: number, action: ActionType) => {
    console.log(id)
    request.post('/api/user/del_role', {
    ...
    <PageContainer title={false}>
      <ProTable<Role, API.PageParams>
        ...
        request={async (params = {}, sort, filter) => {
          ...
          return request<{ data: Role[]; }>('/api/user/role_list', {
            method: 'POST',
            headers: {headers},
            data: params,
          });
      ...
      <Modal title="新增角色" open={isModalVisible} okText="确认" cancelText="取消" forceRender
        ...
        <Form {...layout} name="nest-messages" form={form}>
          ...
          <Form.Item name='name' label="角色名" rules={[{required: true, max: 255},]}>
